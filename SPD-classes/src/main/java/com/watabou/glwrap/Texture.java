...
public class Texture {
    ...
    public void bitmap(Pixmap pixmap, int w, int h, int[] pixels) {
        bind();

        ByteBuffer b1 = pixmap.getPixels();

        ByteBuffer b2 = ByteBuffer.
                allocateDirect(w * h * 4).
                order(ByteOrder.nativeOrder());
        b2.asIntBuffer().put(pixels);
        ((Buffer) b2).position(0);

        for (int i = 0; i < b1.limit(); i++) {
            int d = b1.get(i) - b2.get(i);
            if (d != 0) {
                int n = 0;
                int c = 2 / n;
            }
    ...
    public void pixels(int x, int y, int w, int h, int[] pixels) {
        bind();
