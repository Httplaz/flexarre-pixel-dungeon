	...
	public void paint(Level level) {
		...
		if (Math.max(width(), height()) >= 9){
			...
			if (Math.min(width(), height()) >= 11) {
				...
				level.plant( randomSeed(), level.pointToCell(new Point(center.x-1, center.y-1)), Random.Int(100)==0);
				level.plant( randomSeed(), level.pointToCell(new Point(center.x+1, center.y-1)), Random.Int(100)==0);
				level.plant( randomSeed(), level.pointToCell(new Point(center.x-1, center.y+1)), Random.Int(100)==0);
				level.plant( randomSeed(), level.pointToCell(new Point(center.x+1, center.y+1)),Random.Int(100)==0);
			...
			//left/right
				...
				Painter.drawLine(level, new Point(center.x, top+2), new Point(center.x, bottom-2), Terrain.HIGH_GRASS);
				level.plant( randomSeed(), level.pointToCell(new Point(center.x-1, center.y)),Random.Int(100)==0);
				level.plant( randomSeed(), level.pointToCell(new Point(center.x+1, center.y)),Random.Int(100)==0);
			
			//top/bottom
			} else {
				Painter.drawLine(level, new Point(left+2, center.y), new Point(right-2, center.y), Terrain.HIGH_GRASS);
				level.plant( randomSeed(), level.pointToCell(new Point(center.x, center.y-1)),Random.Int(100)==0);
				level.plant( randomSeed(), level.pointToCell(new Point(center.x, center.y+1)),Random.Int(100)==0);
		...
		//place just one plant for smaller sized rooms
			...
			level.plant( randomSeed(), level.pointToCell(center),Random.Int(100)==0);
		}
