...
public class CharSprite extends MovieClip implements Tweener.Listener, MovieClip.Listener {
    ...
    protected Emitter crimsonBurning;
    ...
    public RectF getDeathFrame() {
        return die.frames[die.frames.length - 1];
    }
    ...
    public void add(State state) {
        switch (state) {
            case BURNING:
                ...
                break;
            case CRIMSON_BURNING:
                crimsonBurning = emitter();
                crimsonBurning.pour(CrimsonFlameParticle.FACTORY, 0.06f);
                if (visible) {
                    Sample.INSTANCE.play(Assets.Sounds.BURNING);
                }
                break;
    ...
    public void remove(State state) {
        switch (state) {
            case BURNING:
                ...
                break;
            case CRIMSON_BURNING:
                if (crimsonBurning != null) {
                    crimsonBurning.on = false;
                    crimsonBurning = null;
                }
                break;
            case LEVITATING:
    ...
    public enum State {
        BURNING, CRIMSON_BURNING, LEVITATING, INVISIBLE, PARALYSED, FROZEN, ILLUMINATED, CHILLED, DARKENED, MARKED, HEALING, SHIELDED, HEARTS
    }
