...
import com.shatteredpixel.shatteredpixeldungeon.items.flexarre.Corpse;
...
public class ItemSprite extends MovieClip {
    ...
    public ItemSprite(Heap heap) {
        //if there is a corpse, get its sprite
        super(getHeapTexture(heap));
        view(heap);
    }

    public ItemSprite(Item item) {

        //if it is a corpse, get its sprite
        super(item instanceof Corpse ? ((Corpse) item).texturePath : Assets.Sprites.ITEMS);
        view(item);
    }
    ...
    public static String getHeapTexture(Heap heap) {
        ...
        switch (heap.type) {
            ...
            case FOR_SALE:
                Item item = heap.peek();
                return item instanceof Corpse ? ((Corpse) item).texturePath : Assets.Sprites.ITEMS;
            default:
                return Assets.Sprites.ITEMS;
        }
    }
    ...
    public ItemSprite view(Item item) {
        if (item.image == -1)
            viewCorpse((Corpse) item, item.glowing());
        else
    ...
    public ItemSprite viewCorpse(Corpse corpse, Glowing glowing) {
        if (this.emitter != null) this.emitter.killAndErase();
        emitter = null;
        texture(corpse.texturePath);
        frame(corpse.frame);
        glow(glowing);
        return this;
    }
